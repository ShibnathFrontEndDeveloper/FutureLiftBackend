            ...
            @foreach ($populerBlog as $populerBlogKey => $populerBlogValue)
              <div class="col-lg-4 last_blog_box">
                <div class="blg_box">
                  ...
                  @php
                        $userDetailsAllBlog = App\Helpers::getUserDetails($populerBlogValue->userId);
                    @endphp
                    <a href="" class="title_blog">{{$userDetailsAllBlog->name}}</a> <span class="title_blog">{{date('jS F Y',strtotime($populerBlogValue->created_at))}}</span>
                    <p class="para_blog">{{Str::words(strip_tags($populerBlogValue->content), 30, ' ...')}}</p>
                  </div>
                </div>
                <!-- <div class="like_box_wrap">
                  <div class="like_box border-end">
